    ...
    async def data_backfill():
        ...
        for team in teams:
            ...
            if ftw_team_id is not None:
                cursor.execute("UPDATE Teams SET ftw_team_id = %s WHERE id = %s", ftw_team_id, team['id'])
                conn.commit()

        # Migrate roster
        cursor.execute("SELECT r.*, u.discord_id FROM Roster r JOIN Users u ON (r.player_id = u.id)")
        ...
        for cup in cups:
            ...
            if ftw_cup_id is not None:
                cursor.execute("UPDATE Cups SET ftw_cup_id = %s WHERE id = %s", ftw_cup_id, cup['id'])
                conn.commit()
        ...
        # Migrate fixtures
        cursor.execute("""SELECT f.*, c.ftw_cup_id, t1.ftw_team1_id, t2.ftw_team2.id
                            FROM Fixtures f
